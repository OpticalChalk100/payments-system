...
const { Transaction, FraudAlert, sequelize } = require('../models');
...
class FraudDetectionService {
  ...
  async analyzeTransaction(transaction) {
    try {
      ...
      if (transaction.amount > this.thresholds.maxTransactionAmount) {
      ...
      if (dailyVolume + transaction.amount > this.thresholds.maxDailyVolume) {
  ...
  async getRecipientRisk(walletId) {
    // Implement recipient risk scoring based on:
    // - Transaction history
    // - Age of account
  ...
  async sendAlert(transactionId, fraudCheck) {
    try {
      ...
      const alert = await FraudAlert.create({
        ...
        description: `Suspicious transaction detected. Score: ${fraudCheck.score}, Factors: ${fraudCheck.factors.join(', ')}`
      });
      ...
      await sendEmail({
        ...
        html: `
          ...
          <ul>
            ...
            <li>Recipient: ${transaction.receiver.user.email}</li>
